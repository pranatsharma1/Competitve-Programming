...
tour when we visit it (either moving down from parent vertex or returning from child vertex).
...
// Resource: GFG(https://www.geeksforgeeks.org/euler-tour-tree/)
...
vector<int>Euler(2 * MAXN);     // this stores the euler tour/path of the tree

int ind = 0;                    // denotes the current index to be filled in the euler path
void EulerTree(int node)        // similar to dfs function
{
    vis[node] = true;
    Euler[ind++] = node;
...
void solve()
{
    ...
    EulerTree(1);   // making a dfs call from first node

    // printing the euler tour/path of the tree
