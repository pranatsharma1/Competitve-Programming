...
vector<bool>vis(MAXN);      // visited array

// maxD is the maximum distance we get after applying dfs from a node and maxNode is the node farthest from it
int maxD,maxNode;        
...
void solve()
{
    ...
    dfs(1,0);    // dfs call from node 1

    for(int i=1;i<=n;i++)
        vis[i]=false;       // initializing the visited array for 2nd dfs call

    maxD=-1;
    dfs(maxNode,0);         // dfs call from the farthest node (i.e. the maxNode after first dfs)
